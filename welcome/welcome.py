...
class Welcomer(commands.Cog):
    ...
    async def get_used_invite(self, guild):
        ...
        for i in self.invite_cache[guild.id]:
            if i in update_invite_cache:
                ...
                try:
                    new_invite = next(
                        inv for inv in update_invite_cache if inv.id == inv.id
                    )
    ...
    async def welcomer(self, ctx, channel: discord.TextChannel, *, message):
        ...
        formatted_message = self.format_message(
            ctx.author, message, SafeString("{invite}")
        )
        if formatted_message:
            ...
            await self.db.find_one_and_update(
                ...
                {
                    "$set": {
                        "welcomer": {"channel": str(channel.id), "message": message}
                    }
                },
                upsert=True,
            )
            await ctx.send(
                f"Message sent to {channel.mention} for testing.\nNote: invites cannot be rendered in test message"
            )
        else:
